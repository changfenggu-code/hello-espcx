...
- `peripheral/` 运行在 ESP32-C6 上，提供 Battery Service GATT Server
- `central/` 运行在桌面端，负责扫描、连接、订阅通知并读取特征值
...
- `common/`：两端共享的 BLE 常量
- `peripheral/`：嵌入式 BLE 外设，基于 `esp-hal`、`esp-radio`、`esp-rtos` 和 `trouble-host`
- `central/`：桌面 BLE 中心程序，基于 `btleplug` 和 `tokio`
...
如果你修改广播名、特征值 UUID 或通知流程，建议把它视为跨端变更，并同步检查 `peripheral/` 与 `central/`。
...
- 通过 `peripheral/rust-toolchain.toml` 固定使用 `nightly`
- 目标平台为 `riscv32imac-unknown-none-elf`
- 在 `peripheral/.cargo/config.toml` 中启用了 `build-std = ["alloc", "core"]`
- 入口文件：`peripheral/src/main.rs`
- GATT Server 定义：`peripheral/src/ble_bas_peripheral.rs`

### `central/`

- 是标准的桌面 Rust 程序
- 当前开发流程主要围绕 Windows BLE 场景组织
- 入口文件：`central/src/main.rs`

...
`run_stack()` 是纯粹的 IO 驱动——不断调用 `runner.run()` 处理每个 HCI 事件和 ACL 数据包。它不知道上层在做什么（广播、连接）。生命周期循环负责重连策略、产品任务和 session 结束后的处理。

**内层 `join`** 在同一次连接里并发运行两个任务：
...
两个任务在同一个 `GattConnection` 上并发运行。被动 handler 阻塞等待 GATT 事件；主动任务驱动周期性通知。任一任务退出（如连接断开）session 即结束。
